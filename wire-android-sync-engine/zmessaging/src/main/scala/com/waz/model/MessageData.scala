...
object MessageData extends
  ...
  implicit object MessageDataDao extends Dao[MessageData, MessageId] with StorageCodecs {
    ...
    def findSystemMessage(conv: ConvId, tpe: Message.Type, sender: UserId)(implicit db: DB) =
      iterating(db.query(table.name, null, s"${Conv.name} = '${conv.str}' and ${Type.name} = '${Type(tpe)}' and ${User.name} = '${User(sender)}'", null, null, null, s"${Time.name} DESC"))
